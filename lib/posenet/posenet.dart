...
class _PoseNetPageState extends State<PoseNet>
{
  ...
  {
    print('----------------KeyPoints 진입---------------');
    if (_recognitions == null || _image == null) return []; // picked image 자체가 안됐다면
            ...
            for (int j = 0; j < posenetPoint.length; j++) {
              if (posenetPoint[j]['part'] == 'rightShoulder')
                canvas.drawLine(Offset(
                    posenetPoint[i]['left'] + ax, posenetPoint[i]['top'] + ay),
                    Offset(posenetPoint[j]['left'] + ax,
                        posenetPoint[j]['top'] + ay), paint); // 선을 그림.
